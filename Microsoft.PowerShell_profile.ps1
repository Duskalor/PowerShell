...
# Zellij CWD inheritance (Windows port can't detect CWD via /proc)
if ($env:ZELLIJ) {
    $_zjCwdFile = "$env:TEMP\zellij_cwd.txt"
    $_zjMarker = "$env:TEMP\zellij_session_$($env:ZELLIJ).marker"

    # First pane creates the marker; subsequent panes find it and inherit CWD
    if (Test-Path $_zjMarker) {
        if (Test-Path $_zjCwdFile) {
            $_targetDir = Get-Content $_zjCwdFile -Raw
            if ($_targetDir -and (Test-Path $_targetDir)) {
                Set-Location $_targetDir
            }
        }
    } else {
        New-Item $_zjMarker -Force | Out-Null
    }

    function prompt {
        (Get-Location).Path | Set-Content "$env:TEMP\zellij_cwd.txt" -NoNewline
...
# PASO 2: Extraer sumillas del HTML (HtmlAgilityPack)
...
foreach ($b in $panelBodies) {
    ...
    ".//div[
        normalize-space()='Fecha de Ingreso:' 
        or 
        normalize-space()='Fecha de Resolución:'
